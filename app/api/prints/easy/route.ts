...
async function parseWithGemini(input: string, items: Array<{ id: string; name: string }>): Promise<AiOrder[]> {
...
export async function POST(req: Request) {
  try {
    ...
    const aiOrders = await parseWithGemini(input, items.map((i) => ({ id: i.id, name: i.name })));
    if (!aiOrders.length) {
      return NextResponse.json({ error: "Não consegui identificar itens válidos no texto" }, { status: 400 });
    }
    ...
    for (const order of aiOrders) {
      ...
      if (!Number.isInteger(quantity) || quantity < 1 || quantity > 50) {
        throw new Error(`Quantidade inválida para item ${order.itemName || order.itemId || "desconhecido"} (1-50)`);
      }
    ...
    if (!results.length) {
      return NextResponse.json({ error: "Nenhum item válido encontrado para impressão" }, { status: 400 });
    }

    return NextResponse.json({ ok: true, results });
  } catch (error: any) {
    return NextResponse.json({ error: error?.message || "Falha ao emitir fácil" }, { status: 500 });
  }
}
